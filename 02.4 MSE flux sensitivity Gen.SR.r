...
# error for flux
Flux.Err <- FLQuant(rnorm(niter,mean = 1.027754,sd= 0.3),
...
for (ref in seq(0.01,0.04,0.01)) {
  ...
  Flux.Err <- FLQuant(rnorm(niter,mean = 1.027757,sd= ref),
                      dimnames = list(year=year_seq, iter=1:niter))
  ...
  MSE_fun <- function(x){
    ...
    for(ii in 2:(length(year_seq))){ # starting from 1982
      ...
      mtf.xsa_nn <- advanceStock(
        ...
        sr.residuals = exp(SR.xsa@residuals)[,ac(year_seq[ii]),,,,],
  ...
  save( res,   file=paste("results/Hindsight MSE Gen.SR/Ref ",ref,".Rdata", sep="") )
...
ggplot() +
  geom_line(aes(x=1982:2014,y=as.numeric(ssb(mtf.xsa_nn))),color="red") +
  geom_line(aes(x=1982:2014,y=as.numeric(ssb(gom.xsa))),color="black")
