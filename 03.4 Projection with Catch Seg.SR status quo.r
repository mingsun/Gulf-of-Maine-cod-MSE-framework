...
niter <- 200
nyear <- 6
ncores <- 6 #as.numeric(Sys.getenv("NUMBER_OF_PROCESSORS"))
...
SR.Err <- FLQuant(exp(sample(c(residuals(SR.3rd)), nyear*niter, replace=TRUE)),
...
limit <- data.frame(Year=2015:2020,Item=c(rep("catch",6)),Value=c(325,599,428,1084,1181,1326))
...
MSE_fun <- function(x){
  ...
  for(ii in seq(year_seq)){
    ...
    stf.xsa <- fwd(stf.xsa, ctrl = ctrl_obj, sr = SR.3rd) 
    ...
    mtf.xsa_nn <- advanceStock(
      stock = mtf.xsa_nn, tac = TAC_nn, sr = SR.3rd,
      maxyear = year_seq[ii]-1,
...
# Save the results
save(res, file="results/Projection with Catch Seg.SR.Rdata")
